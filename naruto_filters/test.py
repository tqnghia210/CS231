...
filters_config = {
    'naruto':
        [{'path': "filters/nhat.png",
          'anno_path': "filters/nhat_annotations.csv",
          'has_alpha': True}],
    'frog':
        [{'path': "filters/frog.png",
          'anno_path': "filters/frog_annotations.csv",
          'has_alpha': True},
          {'path': "filters/nhat.png",
          'anno_path': "filters/nhat_annotations.csv",
          'has_alpha': True}],
}
# phát hiện và trích xuất các điểm đặc trưng trên khuôn mặt trong ảnh đầu vào.
def getLandmarks(img):
    mp_face_mesh = mp.solutions.face_mesh
    selected_keypoint_indices = [127, 93, 58, 136, 150, 149, 176, 148, 152, 377, 400, 378, 379, 365, 288, 323, 356, 70, 63, 105, 66, 55, 285, 296, 334, 293, 300, 168, 6, 195, 4, 64, 60, 94, 290, 439, 33, 160, 158, 173, 153, 144, 398, 385, 387, 466, 373, 380, 61, 40, 39, 0, 269, 270, 291, 321, 405, 17, 181, 91, 78, 81, 13, 311, 306, 402, 14, 178, 162, 54, 67, 10, 297, 284, 389]
    ...
    with mp_face_mesh.FaceMesh(max_num_faces=1, static_image_mode=True, min_detection_confidence=0.5) as face_mesh:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Chuyển đổi các điểm chuẩn hóa thành tọa độ hình ảnh
...
def load_filter(filter_name="naruto"):
...
def calculateDistance(img):
    ...
    with mp_face_mesh.FaceMesh(max_num_faces=1, static_image_mode=True, min_detection_confidence=0.5) as face_mesh:
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            for i in selected_keypoint_indices:
                relevant_keypnts.append(face_keypnts[i])
            # Tính khoảng cách giữa điểm [13] và [14]
            point_13 = relevant_keypnts[13]
            point_14 = relevant_keypnts[14]
            distance = np.linalg.norm(point_13 - point_14)
            return distance
    return 0
...
cap.set(3, 840)
cap.set(4, 640)
shield = cv2.VideoCapture("rasengan2.gif")
# Some variables
count = 0
isFirstFrame = True
sigma = 50
...
while True:
    ...
    if not ret:
        ...
        points3 = calculateDistance(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        print(points3)
        ...
        hands = detector.findHands(output, False)
        if hands:
            ...
            if showShield:
                for hand in hands:
                    ...
                    if 1 in detector.fingersUp(hand):
                        output = Overlay(output, shieldImage, cx, cy, handSize)
